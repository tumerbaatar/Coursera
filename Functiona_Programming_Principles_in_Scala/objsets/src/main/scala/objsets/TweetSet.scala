...
class Empty extends TweetSet {
   ...
   * type `java.util.NoSuchElementException`.
   */
  override def mostRetweeted: Tweet = throw new java.util.NoSuchElementException("No tweets in empty set")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def descendingByRetweet: TweetList = {
    ...
    val tail = this.remove(retweeted).descendingByRetweet
    new Cons(retweeted, tail)
  }
}
...
object GoogleVsApple {
  ...
  val containsKeyword: List[String] => TweetSet =
    (keys: List[String]) => TweetReader.allTweets.filter(t => keys.exists(key => t.text.contains(key)))
  lazy val googleTweets: TweetSet = containsKeyword(google)
  lazy val appleTweets: TweetSet = containsKeyword(apple)
